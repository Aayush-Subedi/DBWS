...
} else if ($_POST['type'] == 'items-auto') {
    $search = $_POST['search'];
    $sql = "SELECT Item.name FROM Item  WHERE Item.name LIKE CONCAT('$search' , '%')";
} else if ($_POST['type'] == 'users-auto') {
    $name = $_POST['search'];
    $sql = "SELECT  fname FROM Users  WHERE fname LIKE CONCAT('$name' , '%')";
}
